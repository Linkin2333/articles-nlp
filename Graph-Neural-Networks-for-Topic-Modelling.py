...
class LoaderDocument:
    ...
    def features(self):
        ...
        if num_nodes != num_features:
            ...
            # log().debug(f"features_df: features: {num_features} columns: {num_columns} label: {label_column} \n{df[label_column]} \n{df.columns.values} \n{df.head(1000)}")
            msg = f"Nodes and features length do not match, nodes: {num_nodes} features: {num_features}"
            raise Exception(msg)

        # features_df = features_df[features_df[label].isin(nodes)]

        log().info(f"Analysing features complete")

        self._features = df
        return self._features, label_column
    ...
    def graph(self):
        ...
        # show_graph(graph, title="Provisional Graph", show_details=True, rows=200)

        node_degrees = list(graph.degree())
        distribution = Counter([degree for (node, degree) in node_degrees])
        # log().debug(f"Provisional degree distribution: {distribution.most_common()}")
        ...
        show_graph(graph, title="Filtered (Final) Graph", show_details=True, rows=200)
    ...
    def documents(self):
        """
        Return the list of token-to-document mappings
        """
...
def show_topics(data, num_rows=25, summary=True):
    ...
    for cluster in data.keys():
        items = data[cluster]
        output += f"\n\n Cluster: {cluster}, items: {len(items)}, (showing first {num_rows} rows for each cluster)"
...
def show_document_tokens(documents, num_docs=25):
    """
    Show documents in human readable format
    """
...
def show_document_topics(data, num_docs=25):
    """
    Show documents in human readable format
    """
...
def show_graph(graph, title="Graph", show_details=True, extended=False, rows=5):
    """
    Return a graph in human readable format
    """
...
def create_embeddings(graph, features_df):
...
def create_induction_embeddings(xedges, graph, features_df):
    # Note that after the merge the output new_nodes may be smaller than the original
    # input new nodes (as discerned from the edges) as nodes that already exist in the
    # original graph are removed
    mgraph, new_nodes, new_edges = merge_graph(graph, xedges)

    mfeatures_df = pd.concat([features_df, xfeatures_df])
    mfeatures_df = mfeatures_df.loc[~mfeatures_df.index.duplicated(keep="first")]
    mfeatures_df = mfeatures_df.sort_index(axis=0)

    # Create a generator that serves up nodes for use in embedding prediction / creation
    # Create a stellar graph object for the COMBINED graph/features
    mGs = sg.StellarGraph(mgraph, node_features=mfeatures_df)

    # Create a generator for the graph object and flow the NEW nodes (ie. output from MERGE)
    node_gen = GraphSAGENodeGenerator(mGs, BATCH_SIZE, NUM_SAMPLES, seed=RANDOM_SEED).flow(new_nodes)

    # Get embeddings for inducted (new) nodes
    x_inp, x_out = graphsage.node_model(flatten_output=False)
    induction_embedding_model = keras.Model(inputs=x_inp, outputs=x_out)
    induction_embeddings = induction_embedding_model.predict_generator(node_gen, workers=4, verbose=1)
    induction_embeddings = induction_embeddings[:, 0, :]

    return induction_embeddings, mgraph, new_nodes, new_edges
...
features_df, label = loader.features()
log().info(f"Features completed, shape: {features_df.shape}")

features_df = features_df.sort_index(axis=0)
filtered_words = loader.filtered_words()
documents = loader.documents()
# label_documents = loader.labels()

show_document_tokens(documents)
...
labels = nodes
log().info(f"ORIGINAL Documents: {len(documents.keys())} Labels: {len(labels)} Filtered Words (25 shown): {filtered_words[0:25]}")
...
embeddings, graphsage = create_embeddings(graph, features_df)
...
cluster_data, word_data, document_data = assemble_data(documents, labels, degrees, y_pred, y_prob)
...
show_document_topics(document_data)
a = 1/0

##########
#
# Step 3:
# - create model using a simple logistic regresion model (this can be made much better
#   but I wanted to emphasize the graph modeling rather than prediction)
#
##########

# Create a simple prediction model based upon the embeddings (features) and clusters (targets)
# This model will be used to predict new clusters (topics)
log().info(f"Creating logistic regression model")
clf = create_logreg_model(embeddings, clusters)
a = 1/0

##########
#
# Step 3:
# - load new data ("inducted") and get new graph and features
# - create embeddings for the new data
#
##########

##########
# Induct new data, create embeddings, and predict clusters
##########
log().info("--- Induction run")

num_rows = 100
loader = LoaderDocument()
loader.load(TEST_FILE, OOV_FILE, FIELDS, DATA_FIELD, num_rows=num_rows, seed=100)

xgraph = loader.graph()
xfeatures_df, xlabel = loader.features()
xfeatures_df = xfeatures_df.sort_index(axis=0)
xdocuments = loader.documents()
xfiltered_words = loader.filtered_words()

xnodes = list(xgraph.nodes)
xedges = list(xgraph.edges())
xdegrees = [xgraph.degree(x) for x in xnodes]
log().info(f"INDUCTION Documents: {len(xdocuments.keys())} Labels: {len(xnodes)} Filtered Words (25 shown): {filtered_words[0:25]}")

# Create embeddings for the inducted data
induction_embeddings, mgraph, new_nodes, new_edges = create_induction_embeddings(xedges, graph, features_df)

##########
#
# Step 4:
# - predict the using the previously created logistic regression model
#
##########

##########
# Predict the cluster for the embeddings using the LOGREG model
##########
embeddings_full = np.concatenate((embeddings, induction_embeddings), axis=0)
lr_y_pred = clf.predict(embeddings_full)
lr_y_prob = clf.predict_proba(embeddings_full)

lr_y_pred_counts = pd.Series(lr_y_pred).value_counts().sort_index()
lr_y_pred_values = pd.Series(lr_y_pred).tolist()
lr_y_prob_values = pd.DataFrame(lr_y_prob)
log().info(f"LOGREG: Y Prediction Counts: \n{lr_y_pred_counts}")
log().info(f"LOGREG: Y Prediction Values: \n{lr_y_pred_values}")
# log().info(f"LOGREG: Y Prediction Probabilities: \n{lr_y_prob_values}")

documents_full = dict(list(documents.items()) + list(xdocuments.items()))
(lr_rows, lr_cols) = lr_y_prob.shape
labels_full = labels + new_nodes
if lr_rows != len(labels_full):
    log().debug(f"documents: {len(documents.keys())} xdocuments: {len(xdocuments.keys())} documents_full: {len(documents_full.keys())}")
    log().debug(f"lr_y_prob: {lr_y_prob.shape}")
    log().debug(f"labels: {len(labels)} ilabels: {len(xnodes)} labels_full: {len(labels_full)}")
    raise Exception(f"Mismatch between probabilities (len: {lr_rows}) and labels (len: {len(labels_full)})")

##########
#
# Step 5:
# - predict the data in using the same distance model used to create original clusters
#
##########

# Note that we used the model to get NEW words and their embeddings
# but documents contain new and original words.  So we need to get
# the full data (new plus original) which is required to assess documents
dm = create_distance_model(labels_full, embeddings_full, centers_dict, xmax_distance)

# log().debug(f"INDUCTION: Node/Label Cluster Mapping: \n{idf}")
dm_y_prob = predict_distance_proba_model(dm)
dm_y_pred = predict_distance_model(dm_y_prob)

# Calculate document probabilities
# Note that induction documents will require probabilities for new nodes as well as original nodes
# as input such that the full document is considered (not just probabilities from new nodes)

dm_y_pred_counts = pd.Series(dm_y_pred).value_counts().sort_index()
dm_y_pred_values = pd.Series(dm_y_pred).tolist()
log().info(f"Y Prediction Counts: \n{dm_y_pred_counts}")
log().info(f"Y Prediction Values: \n{dm_y_pred_values}")
...
#
##########
classification = classification_report(lr_y_pred, dm_y_pred)
log().info(f"LOGREG VS DISTANCE Classification Report: \n{classification}")
